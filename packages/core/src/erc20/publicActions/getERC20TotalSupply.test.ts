...
import { getERC20TotalSupply } from "./getERC20TotalSupply.js";
...
test("read total supply", async () => {
  const totalSupply = await getERC20TotalSupply(publicClient, {
    erc20,
  });
  expect(amountEqualTo(totalSupply, createAmountFromString(erc20, "1"))).toBe(
    true,
  );
});
